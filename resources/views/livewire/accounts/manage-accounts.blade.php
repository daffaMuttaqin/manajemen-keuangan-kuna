<div>
    <div class="mb-6 flex justify-between items-center">
        ...
        <button wire:click="createAccount" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 font-medium text-sm">
            Add Account
        </button>
    </div>
    ...
    @if($isModalOpen)
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 max-w-md w-full shadow-xl">
                <h3 class="text-lg font-bold mb-4">{{ $editingAccountId ? 'Edit Account' : 'Add New Account' }}</h3>
                ...
                <form wire:submit.prevent="saveAccount" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select wire:model="account_type" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" wire:model="opening_balance" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                ...
                @forelse($accounts as $account)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button wire:click="editAccount({{ $account->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</button>
                            <button wire:click="toggleActiveStatus({{ $account->id }})" class="{{ $account->is_active ? 'text-orange-600 hover:text-orange-900' : 'text-green-600 hover:text-green-900' }}">
    ...
    <div class="mt-6 p-4 bg-gray-50 border border-gray-200 rounded-md">
        <div class="flex justify-between items-center">
            <span class="text-sm font-medium text-gray-700">Total Active Company Balance:</span>
            <span class="text-lg font-bold text-gray-900">Rp {{ number_format($balanceService->calculateTotalCompanyBalance(), 2, ',', '.') }}</span>
        </div>
    </div>
</div>
